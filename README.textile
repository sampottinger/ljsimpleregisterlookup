...
This project implements Python parsers for both LabJack MODBUS Map Markup and LabJack Scribe Lanaguage. The formal spec for LJMMM, the markup that aids in the construction of the JSON MODBUS map, is available at http://jsfiddle.net/Q2F4V/embedded/result/ and an informal discussion follows. LJSL, the language used with LJScribe, does not currently have a formal spec but a brief discussion also follows.


h2. LJMMM

LabJack MODBUS Map Markup provides automatic enumeration of names and reigsters in LabJack's JSON MODBUS map. While a formal spec is at http://jsfiddle.net/Q2F4V/embedded/result/, LJMMM fields interprets both special fields in register names as well as data types for the automatic enumeration of addresses. The existing "JSON MODBUS map":https://bitbucket.org/labjack/ljm_constants/src/5b43af77eb9273cb4f75af9d2be6c9f858f22c56/LabJack/LJM/ljm_constants.json?at=master already uses LJMMM extensively.
...
"LJScribe":http://ljsimpleregisterlookup.herokuapp.com/scribe reads LabJack Scribe Language to produce HTML output. Basically Scribe looks for @registers: followed by a comma seperated list of LJMMM Field Language entries. Currently scribe limits these fields to a single enumeration. So, AIN#(1:10) is valid but AIN#(1:10)_#(1:10) is not currently supported. An example of input into LJScribe is at https://gist.github.com/Samnsparky/7122519 and the corresponding output is at http://jsfiddle.net/sampottinger/36vCD/.
...
A local web server can be run either through flask or foreman. Foreman:
...
* The page on labjack.com should not need to be changed but, if it does, use the code in drupal_snippet.html_snippet. Be aware that Drupal's TinyMCE editor may try to change the HTML. The non-visual editor is suggested.
